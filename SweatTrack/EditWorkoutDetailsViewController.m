...
- (void)sendAnalyticsData
{
    ...
    if (![defaults objectForKey:@"analyticsEditWorkout"]) {
        [[[GAI sharedInstance] defaultTracker] trackView:@"Edit Workout"];
        [defaults setObject:[NSDate date] forKey:@"analyticsEditWorkout"];
    }
    else {
        NSDate *analyticsLastPostDate = [defaults objectForKey:@"analyticsEditWorkout"];
