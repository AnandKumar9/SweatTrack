...
- (void)sendAnalyticsData
{
    ...
    if (![defaults objectForKey:@"analyticsAddWorkoutTemplateDefaultValue"]) {
        [[[GAI sharedInstance] defaultTracker] trackView:@"Set Default Value in Workout Template"];
        [defaults setObject:[NSDate date] forKey:@"analyticsAddWorkoutTemplateDefaultValue"];
    }
    else {
        NSDate *analyticsLastPostDate = [defaults objectForKey:@"analyticsAddWorkoutTemplateDefaultValue"];
