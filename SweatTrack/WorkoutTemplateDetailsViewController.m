...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (ANALYTICS_TRACKING == TRUE) {
        [self sendAnalyticsDataForParameter:self.sourceSegueIdentifier];
    }
}
...
- (void)sendAnalyticsDataForParameter:(NSString *)parameter
{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    
    NSString *analyticsDefaultsKey;
    NSString *analyticsParameter;
    
    if ([parameter isEqualToString:@"Add Workout Template"]) {
        analyticsDefaultsKey = @"analyticsCreateWorkoutTemplate";
        analyticsParameter = @"Create Workout Template";
    }
    else if ([parameter isEqualToString:@"Show Workout Template Details"]) {
        analyticsDefaultsKey = @"analyticsViewWorkoutTemplate";
        analyticsParameter = @"View Workout Template";
    }

    
    if (![defaults objectForKey:analyticsDefaultsKey]) {
        [[[GAI sharedInstance] defaultTracker] trackView:analyticsParameter];
        [defaults setObject:[NSDate date] forKey:analyticsDefaultsKey];
    }
    else {
        NSDate *analyticsLastPostDate = [defaults objectForKey:analyticsDefaultsKey];
