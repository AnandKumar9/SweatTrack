...
- (void)sendAnalyticsData
{
    ...
    if (![defaults objectForKey:@"analyticsViewWorkoutsInMonth"]) {
        [[[GAI sharedInstance] defaultTracker] trackView:@"View Workouts In Month"];
        [defaults setObject:[NSDate date] forKey:@"analyticsViewWorkoutsInMonth"];
    }
    else {
        NSDate *analyticsLastPostDate = [defaults objectForKey:@"analyticsViewWorkoutsInMonth"];
